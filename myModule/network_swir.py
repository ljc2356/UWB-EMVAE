...
class UWBSwinIR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.is_attention == True:
            x_recon_Amp = torch.sqrt(torch.square(input=x_recon[:, 0, :, :]) + torch.square(
                input=x_recon[:, 1, :, :]))  # (N,2,8,50) -> (N,8,50)
            x_recon_Amp = x_recon_Amp.view(x_recon_Amp.shape[0], 1, x_recon_Amp.shape[1],
                                           x_recon_Amp.shape[2])  # add the dimension of channel (N,8,50)->(N,1,8,50)
            ...
            return x_reconAttention
        else:
            return x_recon


class UWBSepNet(nn.Module):
    def __init__(self, is_attention=True):
        super(UWBSepNet, self).__init__()
        self.UWBSwinIR_1 = UWBSwinIR(is_attention=True)
        self.UWBSwinIR_2 = UWBSwinIR(is_attention=True)
        self.UWBSwinIR_3 = UWBSwinIR(is_attention=True)

    def forward(self, x):
        MPC1, _ = self.UWBSwinIR_1(x)
        MPC2, _ = self.UWBSwinIR_2(x)
        MPC3, _ = self.UWBSwinIR_3(x)

        MPC_list = torch.cat((MPC1.unsqueeze(dim=1), MPC2.unsqueeze(dim=1), MPC3.unsqueeze(dim=1)), dim=1)
        recSig = (MPC1 + MPC2 + MPC3) / 3
        return MPC1, MPC2, MPC3, MPC_list, recSig
